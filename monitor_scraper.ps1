while ($true) {
  try {
    if (Test-Path "data/brutos/ram/detalle_ram.json") {
      $content = Get-Content "data/brutos/ram/detalle_ram.json" -Raw -ErrorAction Stop
      ...
      $sizeMB = [math]::Round((Get-Item "data/brutos/ram/detalle_ram.json").Length / 1MB, 1)
